...
ThisBuild / developers := List(
  tlGitHubDev("bbuchsbaum", "Bradley Buchsbaum")
)
// The remote is a custom SSH host alias (github-bbuchsbaum), which sbt-typelevel cannot parse as a
// github.com URL, so it derives neither of these; they are stated instead.
ThisBuild / homepage := Some(url("https://github.com/bbuchsbaum/storymodel4s"))
ThisBuild / scmInfo := Some(
  ScmInfo(
    url("https://github.com/bbuchsbaum/storymodel4s"),
    "scm:git:git@github.com:bbuchsbaum/storymodel4s.git"
  )
)
...
// CI is generated by sbt-typelevel (`sbt githubWorkflowGenerate`; `githubWorkflowCheck` runs on
// CI, so regenerate after editing anything above or below). Publishing is off until T7 settles the
// MiMa baseline (the coordinate and scmInfo were settled on 2026-09-17): a publish job with no
// Sonatype secrets would fail on every push to main. Source files carry no license headers, so the
// header check is off. The dependency-graph job stays off until a first green run establishes a
// baseline.
...
// published artifacts; without an override sbt clones github.com/canardlapin/grakern at the pinned
// revision into its staging area (this is what CI does). A local checkout may be substituted with
// `-Dstorymodel4s.grakern.build=/path/to/grakern` (or the environment variable
// STORYMODEL4S_GRAKERN_BUILD); evidence for the pin must come from a run WITHOUT the override.
// grakern's own build pins graph4s and gale by SHA from GitHub.
