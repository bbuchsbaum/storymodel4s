...
// coordinate, scmInfo and the MiMa baseline: a publish job with no Sonatype secrets would fail on
// every push to main. Source files carry no license headers, so the header check is off. The
// dependency-graph job stays off until a first green run establishes a baseline.
