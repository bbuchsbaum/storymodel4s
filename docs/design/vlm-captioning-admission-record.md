...
- **Weights.** `Qwen/Qwen3-VL-4B-Instruct` at revision `ebb281ec70b0` fetched with `huggingface_hub` into the ignored `tmp/models/`; both shards hash to the repository's LFS digests (`model-00001-of-00002.safetensors` `30a01a0556622645a3cce87b655bbbbbc1f170c196099f1b666c93202c3339a9`, `model-00002-of-00002.safetensors` `046296a2a387efb43b0c997d5833c789604d168834f6e0d3064bf7bb13d002a6`); `config.json` `edac7703329133edfc53e46ac0081835144c99d7eebf28b71c732694d435224d`. The worker re-verifies the shards before every load. Not committed; 8.3 GB.
- **Runtime.** `media/worker-vlm/` (uv-locked: `mlx-vlm` 0.6.17, `mlx` 0.32.2, `huggingface-hub`, `numpy`, `pillow`, `jinja2`; CPython 3.13.11; Apple M3 Max, 36 GB). Worker `caption_worker.py`, sha256 `86ef031a29fdfa68b2ed57f21e03613b73b2cefd37e94a44720bc3fb6c56d18a`; runtime line `mlx_vlm 0.6.17 mlx 0.32.2 python 3.13.11`, observed independently of outcomes by `WorkerRealization.observeCaption`. Offline.
- **Preprocessing identity as applied (read back).** patch 16, temporal patch 2, merge 2, mean and std 0.5; `minPixels` 65536, `maxPixels` 1048576 as declared; each 32x18 F0 frame became a 12x22-patch grid (one temporal slot); prompt digest `5b2c6f12a2b37c81226f7f620800bd0a73cd3a46ccb02b46985b6642458bd3f5`; greedy, `maxNewTokens` 96, seed 0; frames as an image sequence.
- **F0 result.** Three extents (four sampled frames each from the three declared shots): "a single, solid red screen…", "a single, solid green screen…", "a single, solid, deep blue screen…"; 319 prompt tokens and 23–26 generated tokens each; two runs byte-identical; outcome sha256 `c2f69ccb9086bd55a7995ee6250504c6d976aa0d50c701732fcf6a87e0367d0b`. Supports from the packet index: [0, 16000), [16000, 32000), [32000, 48000) ticks on 1/24000.
- **Courts.** `media/src/test/scala/storymodel4s/media/CaptionSearchSuite.scala` (7), replaying the recorded envelope in ordinary CI and running the worker live when the venv and weights exist.
- **Not established.** Anything about film content; the 8B arm (not realized: 17.5 GB against 13 GB free); no calibration; no `TimedSegment` adapter; interpreter and wheel bytes unhashed; the read-back `applied` values trust `mlx-vlm`'s processor object; no container or SBOM.
