...
All terminal provider/consumer command logs, fresh JUnit files, four smoke attempts
and their screenshots/DOM/lifecycle diagnostics are retained in the
[release archive](release-artifacts.tar.gz), with 669 individually hashed members in
its [manifest](release-artifact-manifest.json). Failed attempts retain their failed
status and are not counted as qualification.
...
the real sibling repository. Its final commit `b156029` retains provider pin `9dcaf8e7`.
The [consumer gate](consumer-gate.json) binds all four actual clone revisions separately
from generated Pins: eight aliases, 245 passed, no failures/errors/skips, clean before
and after, plus compilation, formatting and production linking. The [consumer gate runner](consumer-gate.py) executes all eight
...
reconciles all seven ticket criteria to the completed local courts.

The original browser smoke failed on both the [S4b candidate](candidate-original-smoke.json)
and [pre-S4b baseline](baseline-original-smoke.json): 237 checks passed, a stale
`1600x420px` expectation failed, and a proxy-group center click timed out on the SVG
background. Both owned browsers closed cleanly. The [test-only consumer repair](consumer-smoke-repair.json)
uses the pinned EditionSpec `1600x1080px` and the documented focus + Shift+Enter path,
retaining existing pointer tests and every composite selection/identity/style/ARIA
assertion. Its passive DOM diagnostic confirms the center hits the background.
The repaired [baseline](baseline-repaired-smoke.json) and [final S4b consumer](consumer-smoke.json)
each pass all 260 checks. Both global audits return zero, and exact launched PID plus
owned-cache process snapshots are empty afterward. The independent cold repair review
has no remaining source finding. No application production behavior changed for this repair.
...
For the consumer, use standalone clones at `b156029` (storyatlas4s), `9dcaf8e7`
(storymodel4s), `4eb566d9208f474d64d61e778e084dee2ddbaa76` (intaglio), and
`0329c43c88a0b71e9aa4456723bb16bac2fa3841` (grakern). Set `D1A_S4B_CONSUMER_ROOT`
and a fresh `D1A_S4B_OUTPUT`, then run the final linked `consumer-gate.py` and
`consumer-smoke.py` from this evidence directory. Install the consumer's locked
Playwright 1.55.1 package and its Chromium into `<consumer-root>/playwright-browsers`
using Node 22; the recorded Node 26 installer attempt stalled during extraction,
was stopped only after ownership checks, and remains preserved separately. The
successful Node 22 installer and both actual browser executable hashes are recorded.
The smoke runner requires a fresh edition directory and audits browser ownership
before and after automation, including the headless-shell PID omitted by the global
guard's name filter. Do not replace that check with the global guard alone.
