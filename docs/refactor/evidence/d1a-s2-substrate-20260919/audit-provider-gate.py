...
created = getattr(log.stat(), 'st_birthtime', log.stat().st_mtime - receipt['seconds'] - 2)
time_basis = 'log file creation timestamp' if hasattr(log.stat(), 'st_birthtime') else 'log modification minus recorded duration minus 2 seconds'
...
for task, line in zip(tasks, actual_lines):
    ...
    for report in reports:
        assert report.stat().st_mtime >= created, (task, 'stale JUnit', str(report))
