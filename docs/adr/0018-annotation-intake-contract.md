...
is the single declaration of TR duration and participant aliases/exclusions consumed by
Scala `SherlockSceneCoding` and the Python scene scorer. It also declares the Python scorer's
scene count and existing closed-interval, first-sorted-match convention; this slice does not
change Scala's interval lookup or claim interval-classification parity. Corpus descriptors
may assert agreement but cannot override the shared TR and alias rules. An actual Scala export
is checked against Python observations and the preregistered numeric oracle. Changed aliases
or TR values must fail the witness.
