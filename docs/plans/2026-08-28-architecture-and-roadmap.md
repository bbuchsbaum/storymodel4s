...
| `features` | core | **aligned feature tracks** (design record §106–117): `FeatureSpace[V]`, typed targets (token/sentence/situation/segment/boundary/turn), `Estimate`/`ScoreEstimate` with missingness, `FeatureTrack[T, V]` bound to exact `SpanSet` support, `TokenClass`/lexical view + `WindowPlan` (basis, width, step, edge policy) over the surface axis, declared `WindowReducer`s (sum, mean, weighted mean, max, variance, slope, kernel smoothing) that retain eligible/observed counts and coverage, `FeatureDerivation` recipes forming a content-addressed DAG, sidecar manifests/refs, `BoundaryTarget`/`BoundaryEvidence`, `WorldTimeTransition`, and a **feature-use ledger** so boundary/hierarchy builds record their input spaces (anti-circularity, §116). Raw provider tracks are immutable; missing is never silently zero |
