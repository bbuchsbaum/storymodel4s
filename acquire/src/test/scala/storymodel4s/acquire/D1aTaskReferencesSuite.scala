...
class D1aTaskReferencesSuite extends FunSuite:
  ...
  private val valid = TaskReferences(
    surface.sentences.map(_.id),
    surface.tokens.map(_.id),
    Vector("unchecked-node"),
    Vector(ClaimId.unsafe("unchecked-claim"))
  )
  private val unknown = SurfaceUnitId.unsafe("unknown")
  private val cases = Vector(
    valid,
    TaskReferences.empty,
    valid.copy(sentenceIds = Vector(unknown)),
    valid.copy(tokenIds = Vector(unknown)),
    valid.copy(sentenceIds = Vector(surface.tokens.head.id)),
    valid.copy(tokenIds = Vector(surface.sentences.head.id)),
    valid.copy(
      sentenceIds = Vector(unknown, surface.tokens.head.id),
  ...
  test("anchored references refuse missing and wrong-kind parser units"):
    ...
    assert(
      valid
        .copy(sentenceIds = Vector(SurfaceUnitId.unsafe(film.id.value)))
        .validateAgainst(atlas)
        .isInvalid
    )
  ...
  test("absence of a bound proposal surface refuses all parser references"):
    ...
    assert(
      TaskReferences.empty
        .copy(sentenceIds = Vector(surface.sentences.head.id))
        .validateAgainst(noSurface)
        .isInvalid
    )
    assert(
      TaskReferences.empty
        .copy(tokenIds = Vector(surface.tokens.head.id))
        .validateAgainst(noSurface)
        .isInvalid
    )
