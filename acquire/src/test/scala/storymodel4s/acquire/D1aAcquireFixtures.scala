...
private[acquire] object D1aAcquireFixtures:
  ...
  val bound = BoundProposalSurface.of(
    surface,
  ...
  def inlineEvidence(id: String, text: Boolean = false, anchors: Boolean = false): EvidenceRef =
    EvidenceRef.Inline(
      Fixtures
        .evidence(id, Option.when(text)(Fixtures.someSpans))
  ...
  def bundle(
      proposals: Vector[AgentProposal[Int]],
      support: Option[TypedSupport] = None,
      basis: AcceptanceBasis = determined
  ): EvidenceBundle[Int] =
    EvidenceBundle(
      proposals,
      Vector.empty,
      StructuralValidity.Valid,
      SourceSupport(1.0, support),
      1.0,
      Vector(CandidateBasis(1, basis))
    )
