...
/** Differential court against the exact pre-migration Resolver, not an old-binary replay. Fixtures
  * contain text support only. The historical object is separately hash-bound.
  */
class D1aTextResolverParitySuite extends FunSuite:
  ...
  private def same(
      b: EvidenceBundle[Int],
      fp: FamilyPolicy,
      clue: String,
      f: ClaimFamily = family
  ): Unit =
  ...
  test("all 8640 declared text-policy cases preserve complete resolution states"):
    ...
    do
      ...
      val bases = basisIndex match
        ...
        case n =>
          val probabilities = Vector(
            fp.reviewBand.value - 0.01,
            fp.reviewBand.value,
            (fp.reviewBand.value + fp.acceptThreshold.value) / 2.0,
            fp.acceptThreshold.value,
            fp.acceptThreshold.value + 0.01
          )
      ...
      val findings = critic match
        ...
        case _ =>
          Vector(
            Fixtures.finding(FindingCode.HallucinatedConcept, score = Some(fp.criticBlockThreshold))
          )
      val b = EvidenceBundle(
        proposals,
        findings,
        structural,
        SourceSupport
          .text(score, Option.when(supportForm == 1 || supportForm == 3)(Fixtures.someSpans)),
        1.0,
        bases
      )
      same(
        b,
        fp,
        s"policy=$fp basis=$basisIndex pattern=$pattern support=$supportForm score=$score structural=$structural critic=$critic"
      )
      checked += 1
    assertEquals(checked, 8640)

  test("text controls retain no-proposal precedence and unresolved ledger references"):
    val noProposals = Vector(
      Vector.empty[AgentProposal[Int]],
    ...
    for proposals <- noProposals; fp <- Vector(FamilyPolicy.Ordinary, FamilyPolicy.Conservative) do
      ...
      same(
        b.copy(findings = Vector(Fixtures.finding(FindingCode.HallucinatedConcept))),
        fp,
        "critic precedence"
      )
    ...
    same(
      b.copy(sourceSupport = SourceSupport.text(1.0, Some(Fixtures.someSpans))),
      FamilyPolicy.Ordinary,
      "ById with bundle spans"
    )

  test("text losing support and losing calibration cannot license the leader"):
    val b = bundle(
      Vector(
        proposal(1, inlineEvidence("one"), "one"),
        proposal(1, inlineEvidence("two"), "two"),
        proposal(2, inlineEvidence("losing", text = true), "three")
      )
    )
    same(b, FamilyPolicy.Ordinary, "only losing support")
    same(
      b.copy(bases = Vector(CandidateBasis(2, calibrated(0.99)))),
      FamilyPolicy.Ordinary,
      "only losing calibration"
    )
  ...
  test("text rank ties and duplicate evidence retain complete ordering"):
    ...
    for
      ...
      bases <- Vector(
        Vector.empty,
        Vector(CandidateBasis(1, determined), CandidateBasis(2, determined))
      )
    do same(bundle(proposals).copy(bases = bases), FamilyPolicy.Ordinary, s"ties $proposals $bases")
    val repeated = AgentProposal.proposed(
      Fixtures.task,
      1,
      NonEmptyVector.of(z, a, z),
      None,
      Vector.empty,
      Fixtures.receipt
    )
  ...
  test("text threshold neighbors and critic score boundaries preserve outcomes"):
    for fp <- Vector(FamilyPolicy.Ordinary, FamilyPolicy.Conservative, FamilyPolicy.Development) do
      val b = bundle(
        Vector(
          proposal(1, inlineEvidence("a", text = true)),
          proposal(1, inlineEvidence("b", text = true), "two")
        )
      )
      for
        threshold <- Vector(fp.reviewBand.value, fp.acceptThreshold.value)
        p <- Vector(java.lang.Math.nextDown(threshold), threshold, java.lang.Math.nextUp(threshold))
      do same(b.copy(bases = Vector(CandidateBasis(1, calibrated(p)))), fp, s"probability=$p")
      for score <- Vector(
          None,
          Some(java.lang.Math.nextDown(fp.criticBlockThreshold)),
          Some(fp.criticBlockThreshold),
          Some(java.lang.Math.nextUp(fp.criticBlockThreshold))
        )
      do
        same(
          b.copy(findings =
            Vector(Fixtures.finding(FindingCode.HallucinatedConcept, score = score))
          ),
          fp,
          s"critic=$score"
        )
  ...
  test(
    "text upstream-only evidence and every declared family preserve default and override policy"
  ):
    val upstream = EvidenceRef.Inline(
      Fixtures
        .evidence("upstream")
    ...
    val families = Vector(
      ...
      ClaimFamily.Custom("d1a", "custom")
    )
    for f <- families do
      assertEquals(
        Resolver.resolve(f, b, AcceptancePolicy.Conservative),
        BaselineTextResolver.resolve(f, b, AcceptancePolicy.Conservative),
        f.toString
      )
      for fp <- Vector(FamilyPolicy.Ordinary, FamilyPolicy.Conservative, FamilyPolicy.Development)
      do same(b, fp, s"override=$f/$fp", f)
