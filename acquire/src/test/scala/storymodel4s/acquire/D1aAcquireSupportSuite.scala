...
class D1aAcquireSupportSuite extends FunSuite:
  ...
  test("accepting control: existing text support retains spans and acceptance"):
    ...
    assert(resolve(bundle(Vector(proposal(1, inline("text", text = true))), text.support)).isAccepted)
  ...
  test("inline singular support covers neither, text, anchors and twin form without selecting a winner"):
    assertEquals(inline("none").support, None)
    assertEquals(inline("text", text = true).support, Some(TypedSupport.Text(Fixtures.someSpans)))
    assertEquals(inline("anchors", anchors = true).support, Some(TypedSupport.Anchored(anchored)))
    val both = inline("both", text = true, anchors = true)
  ...
  test("anchored bundle support alone admits determined and calibrated winners"):
    for basis <- Vector(determined, calibrated(0.95)) do
      val b = bundle(Vector(proposal(1, inline("no-direct"))), Some(TypedSupport.Anchored(anchored)), basis)
  ...
  test("winning inline anchored support alone admits determined and calibrated winners"):
    for basis <- Vector(determined, calibrated(0.95)) do
      val b = bundle(Vector(proposal(1, inline("winning", anchors = true))), basis = basis)
      assert(resolve(b).isAccepted)
      assertEquals(resolve(b.copy(proposals = Vector(proposal(1, inline("missing"))))), missing)

  test("losing inline anchors do not support the winner"):
    val b = bundle(Vector(proposal(1, inline("first"), "one"),
      proposal(1, inline("second"), "two"), proposal(2, inline("loser", anchors = true), "three")))
  ...
  test("twin-form inline evidence alone is ambiguous but does not erase another valid witness"):
    val b = bundle(Vector(proposal(1, inline("both", text = true, anchors = true))))
    assertEquals(resolve(b), missing)
  ...
  test("anchored presence preserves structural critic agreement and calibration boundaries"):
    val b = bundle(Vector(proposal(1, inline("anchor", anchors = true))))
