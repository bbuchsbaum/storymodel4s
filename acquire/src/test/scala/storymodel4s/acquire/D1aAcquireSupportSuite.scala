...
class D1aAcquireSupportSuite extends FunSuite:
  ...
  private def resolve(b: EvidenceBundle[Int]) =
    Resolver.resolve(ClaimFamily.EntityMention, b, policy)
  ...
  test("accepting control: existing text support retains spans and acceptance"):
    ...
    assert(
      resolve(
        bundle(Vector(proposal(1, inlineEvidence("text", text = true))), text.support)
      ).isAccepted
    )
  ...
  test(
    ...
    assertEquals(
      inlineEvidence("text", text = true).support,
      Some(TypedSupport.Text(Fixtures.someSpans))
    )
    assertEquals(
      inlineEvidence("anchors", anchors = true).support,
      Some(TypedSupport.Anchored(anchored))
    )
  ...
  test("ById cannot invent direct support without a ledger"):
    ...
    assert(
      resolve(bundle(Vector(proposal(1, ref)), Some(TypedSupport.Anchored(anchored)))).isAccepted
    )
  ...
  test("anchored bundle support alone admits determined and calibrated winners"):
    for basis <- Vector(determined, calibrated(0.95)) do
      val b = bundle(
        Vector(proposal(1, inlineEvidence("no-direct"))),
        Some(TypedSupport.Anchored(anchored)),
        basis
      )
  ...
  test("winning inline anchored support alone admits determined and calibrated winners"):
    for basis <- Vector(determined, calibrated(0.95)) do
      ...
      assert(resolve(b).isAccepted)
      assertEquals(
        resolve(b.copy(proposals = Vector(proposal(1, inlineEvidence("missing"))))),
        missing
      )
  ...
  test("losing inline anchors do not support the winner"):
    val b = bundle(
      Vector(
        proposal(1, inlineEvidence("first"), "one"),
        proposal(1, inlineEvidence("second"), "two"),
        proposal(2, inlineEvidence("loser", anchors = true), "three")
      )
    )
    assertEquals(resolve(b), missing)
    assert(
      resolve(
        b.copy(sourceSupport = SourceSupport(1.0, Some(TypedSupport.Anchored(anchored))))
      ).isAccepted
    )
  ...
  test("twin-form inline evidence alone is ambiguous but does not erase another valid witness"):
    ...
    assertEquals(resolve(b), missing)
    assert(
      resolve(b.copy(sourceSupport = SourceSupport.text(1.0, Some(Fixtures.someSpans)))).isAccepted
    )
  ...
  test("anchored presence preserves structural critic agreement and calibration boundaries"):
    ...
    assertEquals(
      resolve(b.copy(bases = Vector.empty)),
