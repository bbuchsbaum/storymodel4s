...
class ParticipantCalibrationSuite extends ScalaCheckSuite:
  ...
  private def example(
    ...
    def alignment(id: ConceptId, start: Int, end: Int): PropositionAlignment =
      ...
      val evidence = Evidence(EvidenceId.unsafe(s"e:${id.value}"), Some(spans), Set.empty,
        parser, StageId.unsafe("synthetic-parser"))
      PropositionAlignment(AlignmentTarget.Concepts(NonEmptySet.one(id)), spans, Credence.unmeasured,
        ClaimMeta.unsafe(ClaimId.unsafe(s"c:${id.value}"), EpistemicStatus.SurfaceExplicit,
          Credence.unmeasured, NonEmptyVector.one(evidence), Provenance.deterministic("test", pipeline)))
    val assignment = RoleAssignment(SourceRole.Numbered(0),
      Some(role -> Credence.unsafeRaw(value, by)))
    val chart = PropositionChart.unchecked(Some(root),
      Map(root -> Concept.predicate("act"), filler -> Concept.name(name)),
      Vector(PropositionRelation(root, assignment, ConceptTarget.Node(filler))), Map.empty,
      Vector.empty, Vector(alignment(filler, 0, name.length), alignment(root, name.length + 1, name.length + 6)),
      ChartProvenance(ChartOrigin.Parser(parser), Vector.empty, Vector.empty), Some(unit.id))
    val checked = right(ChartValidator.check(chart))
    val input = right(ChartProposalProvider.input(source, atlas,
      Vector(unit.id -> PropositionEvidence.of(checked)), None, 0L))
  ...
  private def ordinary: Vector[Item] = Vector("Ada", "Bea", "Cora", "Dora").map(example(_)._2)

  test("Beta-Bernoulli oracle: one success and one failure predict one half") {
    val Vector(a, b, c, _) = ordinary: @unchecked
  ...
  test("unresolved is retained and excluded, never a negative or evidence for another story") {
    val Vector(a, b, c, d) = ordinary: @unchecked
  ...
  test("story group and exact source independently prevent training leakage") {
    ...
    assert(corpus(Vector(a, alias), Vector(judgment(a, Verdict.Correct), judgment(alias, Verdict.Correct))).isLeft)
    val differentItemSameText = example("Ada", role = ParticipantRole.Patient,
      group = Some(StoryId.unsafe("renamed")))._2
    assert(corpus(Vector(a, differentItemSameText),
      Vector(judgment(a, Verdict.Correct), judgment(differentItemSameText, Verdict.Correct))).isLeft)
  }

  test("new scorer, score, role, pipeline and observed parser cannot borrow a fitted probability") {
    val Vector(a, b, _, _) = ordinary: @unchecked
    val model = right(fit(data(a -> Verdict.Correct, b -> Verdict.Correct)))
    Vector(example("New", by = ScorerId.unsafe("different")), example("New", value = 0.9),
      example("New", role = ParticipantRole.Patient)).foreach { (_, i) =>
      assertEquals(model.predict(i), Left(Refusal.UnseenCell))
    }
    Vector(example("New", manifest = Checksum.ofText("different")),
      example("New", parserVersion = "2")).foreach { (_, i) =>
  ...
  test("held-out labels cannot influence their fold model or prediction") {
    val Vector(a, b, c, _) = ordinary: @unchecked
    def held(label: Verdict): Fold = right(leaveStoryOut(data(a -> label,
      b -> Verdict.Correct, c -> Verdict.Incorrect))).find(_.story == a.storyGroup).get
  ...
  test("held-out story removes all its items, including alternate versions and unresolved rows") {
    val Vector(a, b, c, _) = ordinary: @unchecked
    val version = example("AdaVersion", group = Some(a.storyGroup))._2
    val study = data(a -> Verdict.Correct, version -> Verdict.Unresolved("ambiguous"),
      b -> Verdict.Incorrect, c -> Verdict.Correct)
  ...
  test("no scorable held-out cells gives absent losses alongside every refusal") {
    ...
    val folds = right(leaveStoryOut(data(a -> Verdict.Correct, b -> Verdict.Correct, c -> Verdict.Incorrect)))
  ...
  test("corpus requires a complete unique matching batch and one protocol") {
    ...
    assertEquals(leaveStoryOut(data(a -> Verdict.Correct, b -> Verdict.Incorrect)),
  ...
  property("row order never changes the fit; label complementation reflects the prediction") {
    forAll { (aCorrect: Boolean, bCorrect: Boolean) =>
      ...
      assertEqualsDouble(right(forward.predict(c)).value + right(complement.predict(c)).value, 1.0, 1e-15)
    }
  }
  ...
  test("calibration reaches the existing resolver with raw score and evidence unchanged") {
    ...
    assertEquals(calibrated.bundle.bases,
      Vector(CandidateBasis(query.role, AcceptanceBasis.Calibrated(Probability.unsafe(0.75), model.id))))
    val high = right(FamilyPolicy.of(Probability.unsafe(0.7), Probability.unsafe(0.5), 1, true, true, true))
    val policy = input.policy.copy(perFamily = input.policy.perFamily + (ClaimFamily.ParticipantRole -> high))
    val rebuilt = right(NarrativeCompilerInput.of(input.source, input.atlas, input.localCharts,
      input.evidence.values, input.situations, input.contexts, input.summary, input.memberships,
      input.causal, input.entityMentions, Vector(calibrated), input.participantCoverage,
      input.circumstances, input.temporal, policy, input.receipt, input.provenance))
    ...
    assertEquals(participant.meta.credence.basis, CredenceBasis.Calibrated(Probability.unsafe(0.75), model.id))
    val strict = policy.copy(perFamily = policy.perFamily + (ClaimFamily.ParticipantRole ->
      right(FamilyPolicy.of(Probability.unsafe(0.9), Probability.unsafe(0.8), 1, true, true, true))))
    assert(!Resolver.resolve(ClaimFamily.ParticipantRole, calibrated.bundle, strict).isInstanceOf[ResolutionState.Accepted[?]])
  }
