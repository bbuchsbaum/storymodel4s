...
import storymodel4s.proposition.{AlignmentTarget, ChartIdentity, ChartOrigin, PropositionChart, CheckState, PropositionRelation, SourceRole}
...
/** Source-bound participant reliability studies; fitting is not a certificate of generalization.
  * See ADR 0013 for the semantic target and the curator's story-group obligation.
  */
object ParticipantCalibration:
  ...
  /** An exact scored candidate extracted from a checked compiler input, with no construction bypass. */
        ...
        case Vector(proposal) if proposal.disposition == ProposalDisposition.Proposed =>
          (proposal.value, proposal.rawScore) match
            case (Some(role), Some(score)) =>
              val chartIds = input.localCharts.map((u, e) =>
                fields(Vector(u.value, chartKey(e.chart)))
              ).sorted
              val producers = input.localCharts.map { (_, e) =>
                fields(Vector(originKey(e.provenance.origin)) ++
                  e.provenance.receipts.map(producerKey).distinct.sorted)
              }.distinct.sorted
              val producer = digest(Vector(input.source.language.value,
                producerKey(proposal.receipt.call), proposal.receipt.promptPackage.name,
                proposal.receipt.promptPackage.version, proposal.receipt.promptPackage.checksum.hex) ++ producers)
              ...
              val id = digest(Vector("participant-calibration-item/v1", input.source.canonicalChecksum.hex,
                pipeline.hex, producer.hex, attempt.situation.sentence.value, attempt.situation.concept.value,
                attempt.filler.sentence.value, attempt.filler.concept.value, roleKey(role),
                score.scorer.value, Score.hexBits(score.value), proposal.taskId.value,
                NarrativeCompiler.renderProviderCall(proposal.receipt.call), proposal.receipt.promptPackage.checksum.hex,
                fields(chartIds), fields(evidence)))
              Right(new Item(id, storyGroup, input.source.canonicalChecksum, pipeline, producer,
                role, score, attempt))
  ...
  def corpus(items: Vector[Item], judgments: Vector[Judgment]): Either[Refusal, Corpus] =
    ...
    if items.isEmpty then invalid("no items supplied")
    ...
    else if judgments.map(_.protocol).distinct.size != 1 then invalid("mixed adjudication protocols")
    else if items.map(i => (i.pipeline, i.producer)).distinct.size != 1 then invalid("mixed pipelines or producers")
      ...
      case _ => false) then invalid("unresolved judgment needs a reason")
    else
      ...
      val checksum = digest(Vector(EstimatorVersion) ++ ordered.zip(labels).map { (i, j) =>
        fields(Vector(i.id.hex, i.storyGroup.value, verdictKey(j.verdict), j.adjudicator.value, j.protocol.hex))
      })
  ...
  final class CellFit private[ParticipantCalibration] (
    ...
    override def equals(other: Any): Boolean = other match
      case that: CellFit => correct == that.correct && incorrect == that.incorrect && stories == that.stories
      case _ => false
    override def hashCode(): Int = (correct, incorrect, stories).##
    override def toString: String = s"CellFit(correct=$correct, incorrect=$incorrect, stories=${stories.size})"
  ...
  final class Model private[ParticipantCalibration] (
    ...
    def predict(item: Item): Either[Refusal, Probability] =
      ...
      else cells.get(Cell(item.role, item.score)).toRight(Refusal.UnseenCell).flatMap { cell =>
    ...
    def calibrate(item: Item): Either[Refusal, ParticipantAttempt] =
      predict(item).map(p => item.attempt.copy(bundle = item.attempt.bundle.copy(
  ...
  def fit(corpus: Corpus): Either[Refusal, Model] =
    val labeled = corpus.items.zip(corpus.judgments).filter((_, j) => j.verdict match
    ...
    if groups < 2 then Left(Refusal.InsufficientStories(2, groups))
      ...
      val cells = labeled.groupBy((i, _) => Cell(i.role, i.score)).toVector.traverse { (key, rows) =>
        val correct = rows.count(_._2.verdict == Verdict.Correct)
        val incorrect = rows.size - correct
        Probability.from((correct.toDouble + 1.0) / (rows.size.toDouble + 2.0))
          .left.map(e => Refusal.InvalidCorpus(e.message))
          .map(p => key -> new CellFit(correct, incorrect, rows.map(_._1.storyGroup).toSet, p))
      }
      cells.map { fitted =>
        new Model(CalibrationModelId.unsafe(s"participant-calibration:${corpus.checksum.hex}"), corpus,
          fitted.toMap, corpus.items.zip(corpus.judgments).collect {
  ...
  final case class HeldOut(item: Checksum, verdict: Verdict, prediction: Either[Refusal, Probability])
  ...
  final class Fold private[ParticipantCalibration] (
    ...
    override def toString: String = s"CalibrationFold(${story.value}, scored=$scored/${outcomes.size})"
  ...
  def leaveStoryOut(data: Corpus): Either[Refusal, Vector[Fold]] =
    val groups = data.items.zip(data.judgments).collect {
      case (i, j) if j.verdict == Verdict.Correct || j.verdict == Verdict.Incorrect => i.storyGroup
    }.distinct
    if groups.size < 3 then Left(Refusal.InsufficientStories(3, groups.size))
    else Right(data.items.map(_.storyGroup).distinct.sortBy(_.value).map { group =>
      ...
      val outcomes = data.items.zip(data.judgments).filter(_._1.storyGroup == group).map { (i, j) =>
        HeldOut(i.id, j.verdict, fitted.flatMap(_.predict(i)))
      }
      val scores = outcomes.flatMap { o =>
        ...
        for p <- o.prediction.toOption; y <- label yield
          ((p.value - y) * (p.value - y), if y == 1.0 then -math.log(p.value) else -math.log1p(-p.value))
      }
      new Fold(group, fitted, outcomes, scores.size,
        Option.when(scores.nonEmpty)(scores.map(_._1).sum / scores.size),
        Option.when(scores.nonEmpty)(scores.map(_._2).sum / scores.size))
    })

  private def producerKey(call: ProviderCall): String = fields(Vector(call.provider, call.model,
    call.version, optionKey(call.promptTemplateVersion)(_.value)))
  ...
  private def spansKey(spans: SpanSet): String = fields(spans.refs.toVector.map(r =>
    fields(Vector(optionKey(r.unit)(_.value), r.span.start.toString, r.span.endExclusive.toString))))

  private def evidenceKey(e: Evidence): String = fields(Vector(e.id.value, e.extractor.value,
    e.stage.value, fields(e.upstream.toVector.map(_.value).sorted), optionKey(e.spans)(spansKey)))

  private def credenceKey(c: Credence): String = fields(Vector(c.score.render, c.basis match
    case CredenceBasis.Uncalibrated => "uncalibrated"
    case CredenceBasis.Determined(rule) => fields(Vector("determined", rule.value))
    case CredenceBasis.Calibrated(p, model) => fields(Vector("calibrated", Score.hexBits(p.value), model.value))))
  ...
  private def relationKey(r: PropositionRelation): String =
    ...
    fields(Vector(r.from.value, source, ChartIdentity.targetKey(r.to, _.value),
  ...
  private def chartKey[C <: CheckState](chart: PropositionChart[C]): String =
    fields(Vector(ChartIdentity.serialize(chart, _.value),
      fields(chart.concepts.toVector.sortBy(_._1.value).map((id, c) => fields(Vector(id.value,
        optionKey(c.gloss)(identity), optionKey(c.frame)(f => fields(Vector(f.namespace, f.id,
          optionKey(f.senseCredence)(credenceKey)))))))),
      fields(chart.relations.map(relationKey).sorted),
      fields(chart.alignments.map { a =>
        val target = a.target match
          case AlignmentTarget.Concepts(ids) => fields(Vector("concepts") ++ ids.toSortedSet.toVector.map(_.value))
          case AlignmentTarget.Relation(r) => fields(Vector("relation", relationKey(r)))
        fields(Vector(target, spansKey(a.spans), credenceKey(a.credence), a.meta.id.value,
          a.meta.status.toString, credenceKey(a.meta.credence),
          fields(a.meta.evidence.toVector.map(evidenceKey).sorted), a.meta.provenance.softwareVersion,
          a.meta.provenance.configHash.hex,
          fields(a.meta.provenance.calls.map(NarrativeCompiler.renderProviderCall))))
      }.sorted), originKey(chart.provenance.origin),
      fields(chart.provenance.receipts.map(NarrativeCompiler.renderProviderCall)),
      fields(chart.provenance.alternatives.map((hash, c) => fields(Vector(hash.hex, credenceKey(c)))))))
