...
/** Whether a feature was checked against the inputs that induced the narrative structure.
  * No use ledger accompanies features-record/v1, so an aggregate cannot claim independence.
  */
...
final class FeatureValue private[view] (
  ...
  def address: Address = Addressable[FeatureAddress].address(FeatureAddress.Observation(space.id, target))
  ...
  def description: String =
    ...
    s"${space.description}; space=${space.id.value}; $outcome; units=${space.units.getOrElse("unspecified")}; " +
      s"support=$spans; coverage=$covered; circularity=$circularity; domain=$bounds; recipe=$recipe; basis=$basis"
...
object FeatureRendering:
  ...
  private[view] def attach(
    ...
    for
      ...
      _ <- require(scene.state.horizon == EpistemicHorizon.Omniscient,
        "materialized features require an omniscient horizon; no prefix-safe measurement was supplied")
      _ <- require(scene.state.feature.contains(selected), "selection does not describe the supplied track")
      _ <- require(selected.resolveSpace == Right(space), "derived output identity does not match its recipe and basis")
      _ <- require(track.provenance.storyChecksum.contains(model.source.canonicalChecksum), "track belongs to another source")
      _ <- require(model.featureSpaces.get(space).contains(track.space), "track space is not the model's declared space")
      _ <- require(model.sidecars.get(space).exists(_.rowCount == observedTargets.size), "track count differs from the model's sidecar")
      _ <- require(refs.map(_.target) == observedTargets && refs.map(_.row) == refs.indices.toVector,
        "observed targets differ from the model's compact sidecar references")
      outcomes <- track.observations.filter(o => FeaturePlanner.atScale(model, scene.featureLayer.scale, o.target))
        .traverse { o =>
          resolver.support(o.target).toRight(DomainError.InvariantViolation("view/features/support", "target has no source support"))
            .flatMap(s => require(o.support.contains(s), "track support differs from its model target").as(o -> s))
        }
      ...
      compiled = outcomes.map { (o, support) =>
        val value = new FeatureValue(track.space, o.target, support, o.estimate, o.coverage,
          track.derivation, track.provenance,
          if track.derivation.isEmpty then FeatureCircularity.NotAggregate else FeatureCircularity.NotAssessed,
          domain)
        val id = VisualIdentity.of(value.address, scene.zoom.narrative,
          MarkId.unsafe(ContentAddress.of("mark", value.address.render, "feature", scene.zoom.narrative.toString)))
        VisualPrimitive.Feature(id, value)
      }
      marks = scene.marks ++ compiled
      navigation <- SceneNavigation.from(marks)
      placements = scene.selectionPlacements ++ (scene.state.selection ++ scene.state.focus).toVector.flatMap { address =>
        NonEmptyVector.fromVector(navigation.marksFor(address)).map(ms => address -> SelectionPlacement.OnMark(ms))
      }
    yield new NarrativeScene(scene.contract, scene.zoom, scene.state,
      new AtlasFeatureLayer(scene.featureLayer.scale, FeatureChannelState.Materialized(selected, space, outcomes.size),
        scene.featureLayer.observations), marks, navigation, placements, scene.provenance)
