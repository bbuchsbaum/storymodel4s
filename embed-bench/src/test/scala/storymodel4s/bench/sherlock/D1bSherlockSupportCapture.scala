...
/** Content-free exact support inventory. The one-unit synthetic inference tests retention only; it
  * is neither a participant replay nor a scientific alignment result.
  */
object D1bSherlockSupportCapture:
  ...
  def main(args: Array[String]): Unit =
    ...
    require(
      built.sourceAtlas.exists(a =>
        a.bundle.identity == library.atlas.bundle.identity &&
          a.units == library.atlas.units
      ),
      "view and library proposal atlas differ"
    )
    ...
    val candidates = Candidates(
      recall.ordered.map(unit => unit.id -> CandidateSet.of(Vector(nominated))).toMap
    )
    ...
    def capture(unit: NarrativeProposalUnit, ref: SourceNodeRef): Json =
      ...
      val feature = node.scoringPosition match
        case Some(ScoringPosition.LegacyAnnotationText(spans)) =>
          Json.obj(
            "kind" -> "LegacyAnnotationText".asJson,
            "refs" -> spans.refs.toVector
              .map(ref =>
                Json.obj(
                  "unit" -> ref.unit.map(_.value).asJson,
                  "start" -> ref.span.start.asJson,
                  "endExclusive" -> ref.span.endExclusive.asJson
                )
              )
              .asJson
          )
    ...
    val rows = library.rows.toVector
      .sortBy(_._1)
      .map((row, unit) => capture(unit, rowRefs(row)).mapObject(_.add("row", row.asJson)))
    val scenes = library.scenes.toVector
      .sortBy(_._1)
      .map((scene, unit) => capture(unit, sceneRefs(scene)).mapObject(_.add("scene", scene.asJson)))
    val axes =
