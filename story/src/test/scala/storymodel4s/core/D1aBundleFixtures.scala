  ...
  def twoAxes(): (SourceBundle, SourceBundle) =
    val seed = right(
      SourceBundle.filmEdition(
        EditionId.unsafe("seed"),
        Checksum.ofText("seed"),
        0L,
        100L,
    ...
    val a = right(
      PresentationAxis.editionPlayback(id, edition.get, 0L, 100L, RationalTimebase.Millisecond)
    )
    val b = right(
      PresentationAxis.editionPlayback(id, edition.get, 0L, 200L, RationalTimebase.Millisecond)
    )
    val streams = Vector(stream(aId, a), stream(bId, b))
    def bundle(axis: PresentationAxis) = right(
      SourceBundle.of(
        edition,
        SourceKind.FilmEdition,
        streams,
