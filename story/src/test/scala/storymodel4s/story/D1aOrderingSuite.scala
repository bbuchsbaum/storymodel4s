...
class D1aOrderingSuite extends FunSuite:
  ...
  private def graph(supports: Vector[SpanSet]): NarrativeGraph =
    built.graph.copy(situations = ids.zip(supports).map((id, support) =>
      id -> withSupport(built.graph.situations(id), support)).toMap)
  private def draft(g: NarrativeGraph) = StoryModel.draft(built.source, built.atlas, g,
    built.hierarchy, DiscourseTrajectory(Vector.empty)).fold(error => fail(error.message), identity)
  ...
  test("equal projected hulls use id independent of map insertion order"):
    ...
    assertEquals(g.discourseOrderOn(bundle), Right(expected))
    assertEquals(g.copy(situations = g.situations.toVector.reverse.toMap).discourseOrderOn(bundle), Right(expected))
  ...
  test("internal graph copy recomputes every ordered model query"):
    ...
    val after = before.copy[ModelStatus.Draft](graph = changed).fold(error => fail(error.message), identity)
