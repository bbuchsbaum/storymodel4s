...
object SherlockSourceAtlas:
  ...
  private def project(tick: Long, segment: CompositionSegment): Either[DomainError, Long] =
    ...
    if tick < source.start || tick > source.endExclusive then
      refuse("coordinate escapes occurrence")
    else
  ...
  def of(input: SherlockAnnotations.Atlas): Either[DomainError, SherlockSourceAtlas] =
    val parts =
      Vector(input.manifest.partA -> input.partABundle, input.manifest.partB -> input.partBBundle)
        .sortBy(_._1.presentationOrdinal)
    if parts
        .map(_._1.presentationOrdinal) != Vector(1, 2) || parts.map(_._1.partId).distinct.size != 2
    then refuse("parts require unique presentation ordinals 1 and 2 and unique identities")
      ...
      val lengths = parts.map { (part, _) =>
        BigInt(part.durationTicks) * rate / part.ticksPerSecond
      }
      ...
      for
        ...
        primary <- SourceBundle.editionPlaybackAxis(
          edition,
          streams,
          authority,
          0L,
        ...
        receipt <- SourceDerivationReceipt.of(
          "sherlock/presentation-composition/v1",
          "manifest ordinal concatenation; exact integral target ticks; native loci retained",
          Vector(input.repairRecord.checksum, input.manifest.annotationSha256) ++ parts.map(
            _._2.identity
          )
        )
        segments <- parts
          .zip(lengths.scanLeft(BigInt(0))(_ + _))
          .zip(lengths)
          .foldLeft[Either[DomainError, Vector[(String, CompositionSegment)]]](
            Right(Vector.empty)
        ...
        mappings <- segments.foldLeft[Either[DomainError, Map[String, TrackComposition]]](
          Right(Map.empty)
        ) { case (acc, (part, segment)) =>
          for
            previous <- acc
            mapping <- TrackComposition.of(
              segment.source.axis,
              primary.id,
              Vector(segment),
              receipt
        ...
        bundle <- SourceBundle.of(
          Some(edition),
          SourceKind.FilmEdition,
          streams,
          primary,
          authority,
          parts.map(p => mappings(p._1.partId))
        )
        rows <- input.rows.foldLeft[Either[DomainError, Map[Int, NarrativeProposalUnit]]](
          Right(Map.empty)
        ) { (acc, row) =>
          for
            previous <- acc
            locus <- input.mediaByRow
              .get(row.row)
              .toRight(
                DomainError.InvariantViolation(
                  "sherlock/source-atlas",
                  s"row ${row.row} has no admitted locus"
                )
              )
            segment <- segments
              .find(_._1 == locus.part)
              .map(_._2)
              .toRight(
                DomainError.InvariantViolation(
                  "sherlock/source-atlas",
                  "row names an unknown part occurrence"
                )
              )
            stream <- streams
              .find(_.nativeAxis == segment.source.axis)
              .toRight(
                DomainError.InvariantViolation("sherlock/source-atlas", "part stream is missing")
              )
            anchors <- locus match
              case SherlockAnnotations.MediaLocus.Extent(_, interval) =>
                ...
                yield Vector(
                  ...
                  EvidenceAnchor.MediaTime(
                    ...
                    stream.id,
                    primary.id,
                    PlaybackIntervalSet.one(projected)
                  )
                )
              case SherlockAnnotations.MediaLocus.Instant(_, at) =>
                ...
                yield Vector(
                  EvidenceAnchor.MediaPoint(bundle.id, stream.id, at),
        ...
        scenes <- input.scenes.foldLeft[Either[DomainError, Map[Int, NarrativeProposalUnit]]](
          Right(Map.empty)
        ) { (acc, scene) =>
          for
            previous <- acc
            support <- EvidenceSupport.of(
              bundle,
              input.rows
                .filter(r => r.row >= scene.firstRow && r.row <= scene.lastRow)
        ...
        atlas <- AnchoredNarrativeAtlas.of(
          bundle,
          rows.toVector.sortBy(_._1).map(_._2) ++ scenes.toVector.sortBy(_._1).map(_._2),
          None
        )
