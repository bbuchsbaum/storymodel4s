...
class D1aProjectionSuite extends FunSuite:
  private val text = SourceBundle.writtenText(StorySource.fromText("A😀B").toOption.get).toOption.get
  ...
  private val film = SourceBundle.filmEdition(EditionId.unsafe("projection-film"),
    Checksum.ofText("film"), 0L, 100L, RationalTimebase.Millisecond).toOption.get
  private val anchored = EvidenceSupport.media(film, film.streams.head.id,
  ...
  test("zero-length and out-of-extent spans remain deferred text validator concerns"):
    Vector(TextSpan.unsafe(1, 1), TextSpan.unsafe(8, 10)).foreach { span =>
      ...
      assertEquals(PrimaryProjection.on(text, TypedSupport.Text(support)),
