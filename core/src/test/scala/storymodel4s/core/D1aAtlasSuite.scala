...
class D1aAtlasSuite extends FunSuite:
  private val film = SourceBundle.filmEdition(EditionId.unsafe("atlas-film"),
  ...
  private val support = EvidenceSupport.media(film, film.streams.head.id,
    PlaybackIntervalSet.one(PlaybackInterval.on(film.primaryAxis, 0L, 20L).toOption.get)).toOption.get
  private val surface = SurfaceAnalyzer.analyze(StorySource.fromText("Alpha beta. Gamma.").toOption.get)
  private val receipt = SourceDerivationReceipt.of("captions", "recipe-one", Vector(Checksum.ofText("film"))).toOption.get
  ...
  test("refuse duplicated bound sentence"):
    ...
    assert(AnchoredNarrativeAtlas.of(film, Vector(unit("one", sentence), unit("two", sentence)), Some(bound)).isLeft)

  test("refuse missing, foreign and nonsentence proposal surfaces"):
    assert(AnchoredNarrativeAtlas.of(film, Vector(unit("one", Some(surface.sentences.head.id)))).isLeft)
    assert(AnchoredNarrativeAtlas.of(film, Vector(unit("one", Some(SurfaceUnitId.unsafe("foreign")))), Some(bound)).isLeft)
    assert(AnchoredNarrativeAtlas.of(film, Vector(unit("one", Some(surface.tokens.head.id))), Some(bound)).isLeft)

  test("refuse support belonging to another bundle"):
    val other = SourceBundle.filmEdition(EditionId.unsafe("other"), Checksum.ofText("other"),
  ...
  test("refuse text and annotation timeline primaries explicitly"):
    ...
    val annotation = SourceBundle.annotationTable(Checksum.ofText("annotation"), 0L, 100L,
  ...
  test("surface binding distinguishes canonical bytes, unit structure and receipt"):
    val changedText = SurfaceAnalyzer.analyze(StorySource.fromText("Omega beta. Gamma.").toOption.get)
    assertNotEquals(bound.checksum, BoundProposalSurface.of(changedText, receipt).checksum)
    val withoutTokens = SurfaceAtlas.of(surface.source, surface.units.filterNot(_.kind == SurfaceUnitKind.Token)).toOption.get
    assertNotEquals(bound.checksum, BoundProposalSurface.of(withoutTokens, receipt).checksum)
    val changedReceipt = SourceDerivationReceipt.of("captions", "recipe-two", receipt.inputChecksums).toOption.get
  ...
  test("bound surface receipt association distinguishes legacy NUL collision"):
    ...
    assertNotEquals(BoundProposalSurface.of(surface, left).identity,
