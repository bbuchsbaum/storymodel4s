...
private[core] object SourceIdentity:
  ...
  def receipt(value: SourceDerivationReceipt): Checksum =
    digest(
      Vector("source-receipt-binding/v1", value.algorithm, value.parameters,
        value.inputChecksums.size.toString) ++ value.inputChecksums.map(_.hex)
    )

  def mapping(value: CheckedMapping): Checksum =
    val payload = value match
      case repair: ClockRepair =>
        Vector("clock-repair", repair.relation.sourceAxis.value, repair.relation.targetAxis.value) ++
      ...
      case composition: TrackComposition =>
        val segments = composition.segments.toVector.map { segment =>
          digest(Vector("segment", segment.occurrence.value) ++ interval(segment.source) ++
            interval(segment.target)).hex
        }.sorted
        Vector("track-composition", composition.relation.sourceAxis.value,
          composition.relation.targetAxis.value, segments.size.toString) ++ segments
      case correspondence: EditionCorrespondence =>
        ...
        Vector("edition-correspondence", correspondence.relation.sourceAxis.value,
          correspondence.relation.targetAxis.value, correspondence.sourceEdition.value,
          correspondence.targetEdition.value, pairs.size.toString) ++ pairs
  ...
  def surface(value: SurfaceAtlas): Checksum =
    val units = value.units.sortBy(_.id.value).map { unit =>
      digest(
        Vector("unit", unit.id.value, unit.kind.toString, unit.span.start.toString,
          unit.span.endExclusive.toString, unit.ordinal.toString) ++ optional(unit.parent.map(_.value))
      ).hex
    }
    digest(Vector("proposal-surface/v1", value.source.id.value, value.source.canonicalChecksum.hex,
      units.size.toString) ++ units)
